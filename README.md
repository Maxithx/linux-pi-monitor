...
The terminal page now keeps saved commands in per-profile collections so you can group recurring tasks (System, Network, Disk, etc.) without losing the quick “Run / Insert” workflow.
...
- Click any row’s **Edit** button to reopen the modal with the existing values. “Run” still sends the command with Enter; “Insert” only types it into the terminal buffer.
...
- Server-side validation trims titles (1–64 chars) and commands (= 2048 chars, reasonable newline count). Nothing auto-runs—Run/Insert still behave exactly like before.
